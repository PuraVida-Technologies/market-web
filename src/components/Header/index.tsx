...
const Header = forwardRef<HTMLInputElement, HeaderProps>(({ isValidating, searchText, setSearchText }, ref) => {
  const router = useRouter();
  const [localSearchText, setLocalSearchText] = useState(searchText);
  const searchTextDebounced = debounce((e: { target: { value: string | string[] | undefined } }) => {
    ...
    <header className={styles.header} ref={ref}>
      <div className={`container ${styles.innerContainer}`}>
        ...
        <div className={styles.inputContainer}>
          ...
          <input value={localSearchText} placeholder="Restaurants" onChange={handleSearchTextChange} />
          {isValidating && (
